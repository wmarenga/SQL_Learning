 /* show variables like 'datadir'; => onde esta localizado o SGBD */

/* Modelagem Básica - 

-- NO MYSQL o comentário é # (é o único diferente).
...
1) CHAR e VARCHAR => caractéres literiais (árvore, joão, matemática);
2) INT E FLOAT => números;
3) BLOB => fotos e textos;
4) TEXT =. textos extensos.
...
/* FORMA 01 - OMITINDO AS COLUNAS => valores na exata ordem das tabelas. */
...
/* FORMA 02 - COLOCANDO AS COLUNAS => valores na ordem das colunas. (NOME,SEXO,ENDERECO,TELEFONE,CPF) */
...
/* ERROR 1264 (22003): Out of range value for column 'CPF' at row 1 =>  INT -2147483648 até 2147483647 */
...
SELECT   EMAIL, SEXO, ENDERECO, NOME, NOW(%y) AS DATA_HORA FROM CLIENTE;
...
SELECT NOME, SEXO FROM CLIENTE
WHERE ENDERECO LIKE '%RJ'; /* no fim da query */

SELECT NOME, SEXO, ENDERECO FROM CLIENTE
WHERE ENDERECO LIKE '%RJ'; /* no fim da query */

SELECT NOME, SEXO, ENDERECO FROM CLIENTE
WHERE ENDERECO LIKE 'OSCAR CURY%'; /* no início da query */

SELECT NOME, SEXO, ENDERECO FROM CLIENTE
WHERE ENDERECO LIKE '%CENTRO%'; /* no meio da query */

/* Resolvendo o exercício  - Seção 4 */
...
/* VERIFICAR QUAL BASE ESTOU CONECTADO */

STATUS;  /* Current database:       livraria */
...
SITUACAO - TRATANDO COM OU / OR (Colocar o que tem MAIOR chance de ser VERDADEIRO = F)
70% MULHERES = SEXO = F
...
SITUACAO - TRATANDO COM E / AND (Colocar o que tem MENOR chance de ser VERDADEIRO = RJ)
70% MULHERES = SEXO = F
30% MORA NO RIO DE JANEIRO
...
-- 21 | Filmes (21/975 = 2% DE VERDADEIRO)   53 | Roupas (53/975 = 5% DE VERDADEIRO), ROUPAS VIRÁ PRIMEIRO
...
/* 
Feminino => 491/975 = 50,35% 
Masculino => 484/975 = 49,64%
*/
...
SELECT NOME, EMAIL, DEPARTAMENTO, CARGO FROM FUNCIONARIOS
WHERE
( DEPARTAMENTO = 'Lar' AND SEXO = 'Feminino' )
OR
( DEPARTAMENTO = 'Filmes' AND SEXO = 'Feminino' );
...
-- Como o MARCULINO tem a maioria, usamos o SEXO primeiro.
...
Exemplo: Rua - Bairro - Cidade - Estado
...
CREATE TABLE ENDERECO(
	...
	/* Relacionamento (1,1) => UNIQUE, (1, n) sem UNIQUE  */
	FK_ID_CLIENTE INT UNIQUE,
	
	/* FK fica na tabela mais fraca */
...
CREATE TABLE TELEFONE(
	...
	/* FK fica na tabela onde a cardinalidade é n */
	FOREIGN KEY(FK_ID_CLIENTE)
	REFERENCES CLIENTE(ID_CLIENTE)
);
...
/*
CHAVE ESTRANGEIRA (FK) É A CHAVE PRIMARIA (PK) DE UMA TABELA
QUE VAI ATÉ A OUTRA TABELA PARA FAZER REFERENCIA ENTRE
REGISTROS (Equiparação de índices). */
...
/* O NULL NA POSICAO DO ID_CLIENTE(PK) DETERMINA PARA O BANCO CRIAR A PK automaticamente, somente no MYSQL. */
...
/* Relacionamento (1, 1) (cliente, telefone) => cardinalidades somente */

/* Ira gerar erro pois o ID_CLIENTE 45 não existe. Gera o ID e apaga.*/
...
/* Relacionamento (1, n) (cliente, endereco)=> cardinalidades somente */
...
SELECT NOME, TIPO, NUMERO /* PROJECAO */
FROM CLIENTE, TELEFONE /* ORIGEM */
...
SELECT FK_ID_CLIENTE, BAIRRO, CIDADE
FROM ENDERECO;
...
WHERE ID_CLIENTE = FK_ID_CLIENTE; /* JUNCAO */
...
FROM CLIENTE 
INNER JOIN TELEFONE
ON CLIENTE.ID_CLIENTE = TELEFONE.FK_ID_CLIENTE
WHERE TIPO = 'RES';
...
FROM CLIENTE 
INNER JOIN ENDERECO
ON CLIENTE.ID_CLIENTE = ENDERECO.FK_ID_CLIENTE
INNER JOIN TELEFONE
ON CLIENTE.ID_CLIENTE = TELEFONE.FK_ID_CLIENTE;
...
ON C.ID_CLIENTE = T.FK_ID_CLIENTE;
...
/* Confirmar sempre antes de atualizar. */

/* ÚNICO */
SELECT * FROM CLIENTE
WHERE ID_CLIENTE IN (6, 7);
...
/* PARA ALTERAR VÁRIOS REGISTROS AO MESMO TEMPO, USAMOS IN (1,2, ETC). */
UPDATE CLIENTE
SET SEXO = 'F'
WHERE ID_CLIENTE IN (6, 7);
...
/* DDL - DATA DEFINITION LANGUAGE => DEFINICAO DO TIPO DE DADO (VARCHAR, INT, FLOAT, ETC)*/
...
/* MELHOR PARA ALTERAR O NOME DA COLUNA */
...
/* MODIFY - ALTERANDO O TIPO => O 'MODIFY' É MELHOR POIS NÃO PRECISAMOS REPETIR O NOME DA COLUNA*/
/* MELHOR PARA ALTERAR O TIPO DA COLUNA */

ALTER TABLE PRODUTO
MODIFY VALOR_UNITARIO VARCHAR(50) NOT NULL;

/* ADICIONANDO COLUNAS NA ÚLTIMA POSIÇÃO */
...
/* EXERCICIOS DML => Manipulação de dados */
...
/* IDENTAR O SELECT POR TABELA (CLIENTE, ENDERECO E TELEFONE)*/
...
/* Acertando o sexo dos IDS => 12 13 18 19  */
...
SELECT C.ID_CLIENTE, C.EMAIL, C.NOME, C.SEXO
FROM CLIENTE C
INNER JOIN ENDERECO E
ON C.ID_CLIENTE = E.FK_ID_CLIENTE
INNER JOIN TELEFONE T
...
/* iNCLUIMOS OS PARENTESES NO "OR" (OR) */
SELECT C.ID_CLIENTE, C.EMAIL, C.NOME, C.SEXO, T.TIPO, E.BAIRRO, E.CIDADE
FROM CLIENTE C
INNER JOIN ENDERECO E
ON C.ID_CLIENTE = E.FK_ID_CLIENTE
...
/* Atenção: A origem sempre será a que tem mais ligações com outras tabelas. Possui a PK (chave primária). */
SELECT   C.NOME, C.EMAIL, T.NUMERO AS CELULAR /* PROJECAO */
FROM CLIENTE C /* ORIGEM */
INNER JOIN ENDERECO E /*JUNCAO */
ON C.ID_CLIENTE = E.FK_ID_CLIENTE /*JUNCAO CONDICAO */
INNER JOIN TELEFONE T /*JUNCAO */
ON C.ID_CLIENTE = T.FK_ID_CLIENTE /*JUNCAO CONDICAO*/
...
/* NÃO É PERMITIDO INSERT COM JOIN */
...
/* NÃO É PERMITIDO DELETE COM JOIN */
...
/* SEM O JOIN O DELETE É PERMITIDO */
DELETE FROM V_JOGADORES
WHERE NOME = 'GUERRERO';

SELECT * FROM V_JOGADORES;

/* SEM O JOIN TAMBÉM CONSEGUIMOS INSERIR */
...
/* ORDENA PELA PRIMEIRA COLUNA E PELA SEGUNDA COLUNA TAMBÉM */
SELECT * FROM ALUNOS
...
/* lÊ PRIMEIRO O ORDER BY E DEPOIS O SELECT (PROJEÇÃO), SÓ TRAZ O NOME */
...
/* ASCENDENTE => PADRÃO É SEMPRE ASCENDENTE */
SELECT * FROM ALUNOS
ORDER BY 1 ASC;
...
/* Não precisa de ; pois é um comando de infraestrutura (não SQL) */
STATUS
...
/*  Usar o novo delimitador $ */
SELECT * FROM V_RELATORIO$

/* O banco ignora os espaços extra */
DELIMITER  @

/* Gera erro, pois precisa de um espaço */
DELIMITER@

/* Cria mas ignorando o que vem depois */
DELIMITER @ @

DELIMITER ;

/* Fechando o banco, o delimiter volta para ; */

/* STORED PROCEDURES */

SELECT 'MAFRA';

/* Temos que alterar o delimiter para o SQL não parar no ; (no meio do procedure) */
DELIMITER $
...
Atenção: Podemos chamar a procedure com o delimitador alterado $, ou alterar
o delimiter primeiro e chamar a procedure com ;
...
ALTER TABLE CURSOS
CHANGE ID_CURSOS ID_CURSOS INT AUTO_INCREMENT;
...
CREATE PROCEDURE CONSULTA_CURSOS(P_ID_CURSO INT)
BEGIN
	SELECT NOME, HORAS, VALOR FROM CURSOS
	WHERE ID_CURSOS = P_ID_CURSO;
END
#

DELIMITER ;

CALL CONSULTA_CURSOS(2);

/* QUANTIDADE DE HOMENS E MULHERES */

DELIMITER #

CREATE PROCEDURE CONTAR_SEXO()
BEGIN
	SELECT SEXO, COUNT(*) FROM CLIENTE
	GROUP BY SEXO;
END
...
/* AVG - TRAZ O VALOR MEDIO DE UMA COLUNA (MÉDIA ARITMÉTICA) */
...
/*TRUNCATE (DETERMINA A QTD DE CASAS DECIMAIS) */
...
SELECT SEXO, SUM(MARCO)+SUM(JANEIRO)+SUM(FEVEREIRO) AS TOTAL_MARCO
FROM VENDEDORES
GROUP BY SEXO;

SELECT NOME, SUM(MARCO)+SUM(JANEIRO)+SUM(FEVEREIRO) AS TOTAL_GERAL
FROM VENDEDORES
GROUP BY NOME;
...
/* CERTO => USAR SUBQUERIES (INNER QUERY) */
/* PEGA O VALOR DA SUBQUERY E O NOME CORRESPONDENTE AO VALOR */
SELECT NOME, MARCO FROM VENDEDORES
WHERE MARCO = (SELECT MIN(MARCO) FROM VENDEDORES);

/* ERRADO */
SELECT NOME, MAX(MARCO) AS VENDEU_MAIS_MARCO
FROM VENDEDORES;
...
/* SOMENTE O ANDERSON VENDEU MAIS QUE A MEDIA DE 85728,89 */
SELECT TRUNCATE(AVG(MARCO),2) FROM VENDEDORES;

SELECT NOME, MARCO FROM VENDEDORES
WHERE MARCO > (SELECT TRUNCATE(AVG(MARCO),2) FROM VENDEDORES);

/* SOMENTE A CLARA VENDEU MAIS QUE A MEDIA DE 99392,74 */
...
/* TODOS OS OUTROS VENDEDORES VENDERAM MENOS QUE A MEDIA DE 99392,74 */
SELECT TRUNCATE(AVG(FEVEREIRO),2) FROM VENDEDORES;
...
--ADICIONANDO UMA PK SEM AUTO_INCREMENT
ALTER TABLE TABELA 
...
/* CONSTRAINTS (LIMITAÇÕES) SEPARADAS, APÓS OS CREATES */
...
/* ESTA AÇÃO DE DAR O DROP NA FK E RE-INSERIR NOVAMENTE É MUITO ÚTIL QUANDO
QUEREMOS INSERIR REGISTROS SEM VERIFICAR A INTEGRIDADE REFERENCIAL COM A TABELA
PRINCIPAL. */
...
PARA REALIZAR SERVIÇOS E OS SEUS RESPECTIVOS DONOS, Sr. JOSÉ MENSIONOU QUE CADA CLIENTE
POSSUI APENAS UM CARRO. UM CARRO POSSUI UMA MARCA. Sr. JOSÉ TAMBÉM QUER SABER AS CORES
DOS CARROS PARA TER IDÉIA DE QUAL TINTA COMPRAR, E INFORMAR QUE UM CARRO PODE TER MAIS
DE UMA COR. Sr. JOSÉ NECESSITA ARMAZENAR OS TELEFONES DOS CLIENTES, MAS NÃO QUER QUE ELES
SEJAM OBRIGATÓRIOS. */
...
ALTER TABLE TELEFONE
CHANGE TIPO TIPO ENUM('CEL','RES','COM');

ALTER TABLE TELEFONE
CHANGE NUMERO NUMERO VARCHAR(30);
...
ALTER TABLE COR
CHANGE COR COR VARCHAR(30) UNIQUE;
...
INSERT INTO CARRO VALUES(NULL,'UNO', 'BND1342', 1);
INSERT INTO CLIENTE VALUES(NULL,'JOSE','M', 1);
INSERT INTO TELEFONE VALUES(NULL,'CEL', '987654321', 1);
INSERT INTO COR VALUES(NULL,'AZUL');
INSERT INTO CARRO_COR VALUES(1,1);

INSERT INTO MARCA VALUES(NULL,'BMW');
INSERT INTO CARRO VALUES(NULL,'X1', 'XCV34569', 2);
INSERT INTO CLIENTE VALUES(NULL,'TEREZA','F', 2);
INSERT INTO TELEFONE VALUES(NULL,'RES','234578902', 2);
INSERT INTO COR VALUES(NULL,'VERDE');
INSERT INTO CARRO_COR VALUES(2,2);

INSERT INTO MARCA VALUES(NULL,'MERCEDES');
INSERT INTO CARRO VALUES(NULL,'GLA', 'SDR45678', 3);
INSERT INTO CLIENTE VALUES(NULL,'LEONARDO','M', 3);
INSERT INTO TELEFONE VALUES(NULL,NULL,NULL, 3);
INSERT INTO COR VALUES(NULL,NULL);
INSERT INTO CARRO_COR VALUES(3,1);

INSERT INTO MARCA VALUES(NULL,'RENAUT');
INSERT INTO CARRO VALUES(NULL,'CLIO', 'MNH4378', 4);
INSERT INTO CLIENTE VALUES(NULL,'RENATA','F', 4);
INSERT INTO TELEFONE VALUES(NULL,'RES','2346934', 4);
INSERT INTO COR VALUES(NULL,'PRATA');
INSERT INTO CARRO_COR VALUES(4,5);

INSERT INTO MARCA VALUES(NULL,'PEUGEOT');
INSERT INTO CARRO VALUES(NULL,'208', 'BOST6666', 5);
INSERT INTO CLIENTE VALUES(NULL,'MARCEL','M', 5);
INSERT INTO TELEFONE VALUES(NULL,'COM','3245682', 5);
INSERT INTO COR VALUES(NULL,'AMARELO');
INSERT INTO CARRO_COR VALUES(5,6);

INSERT INTO MARCA VALUES(NULL,'LEXUS');
INSERT INTO CARRO VALUES(NULL,'NX', 'LUXO7593', 6);
INSERT INTO CLIENTE VALUES(NULL,'OTAVIO','M', 6);
INSERT INTO TELEFONE VALUES(NULL,'CEL','97459966', 6);
INSERT INTO COR VALUES(NULL,'VERMELHO');
INSERT INTO CARRO_COR VALUES(6,7);

INSERT INTO MARCA VALUES(NULL,'JEEP');
INSERT INTO CARRO VALUES(NULL,'COMPAS', 'SUSP4729', 7);
INSERT INTO CLIENTE VALUES(NULL,'CHRISTIANA','F', 7);
INSERT INTO TELEFONE VALUES(NULL,'CEL','865909867', 7);
INSERT INTO TELEFONE VALUES(NULL,'COM','345768990', 7);
INSERT INTO COR VALUES(NULL,'VERDE OLIVA');
INSERT INTO CARRO_COR VALUES(7,8);

INSERT INTO MARCA VALUES(NULL,'TESLA');
INSERT INTO CARRO VALUES(NULL,'MODELO Y', 'TESL9854', 8);
INSERT INTO CLIENTE VALUES(NULL,'ELON','M', 8);
...
INSERT INTO CARRO VALUES(NULL,'ONIX', 'ONIX9987', 9);
INSERT INTO CLIENTE VALUES(NULL,'RUFF','M', 9);
INSERT INTO TELEFONE VALUES(NULL,'CEL','87652388', 9);
INSERT INTO COR VALUES(NULL,'MARROM');
INSERT INTO CARRO_COR VALUES(9,10);

INSERT INTO MARCA VALUES(NULL,'JAGUAR');
INSERT INTO CARRO VALUES(NULL,'E-PACE', 'JAGA48765', 10);
INSERT INTO CLIENTE VALUES(NULL,'LILICA','F', 10);
INSERT INTO TELEFONE VALUES(NULL,NULL,NULL, 10);
INSERT INTO COR VALUES(NULL,'CARAMELO');
INSERT INTO CARRO_COR VALUES(10,11);

INSERT INTO MARCA VALUES(NULL,'VOLVO');
INSERT INTO CARRO VALUES(NULL,'XC-90', 'VOLV8456', 11);
INSERT INTO CLIENTE VALUES(NULL,'DAMARES','F', 11);
INSERT INTO TELEFONE VALUES(NULL,NULL,NULL, 11);
INSERT INTO CARRO_COR VALUES(11,1);

INSERT INTO CARRO VALUES(NULL,'GT', 'XYZ8456', 5);
INSERT INTO CLIENTE VALUES(NULL,'FERNANDO','M', 12);
INSERT INTO TELEFONE VALUES(NULL,'RES','35123567', 12);
INSERT INTO TELEFONE VALUES(NULL,'CEL','89458234', 12);
INSERT INTO TELEFONE VALUES(NULL,'COM','34005412', 12);
INSERT INTO CARRO_COR VALUES(12,7);

SELECT M.MARCA, CA.MODELO, CO.COR, CA.PLACA, CL.NOME, CL.SEXO, T.TIPO, T.NUMERO
FROM CARRO CA
INNER JOIN MARCA M
ON ID_MARCA = FK_ID_MARCA
INNER JOIN COR CO
ON ID_COR = ID_CARRO
INNER JOIN CLIENTE CL
ON ID_CARRO = FK_ID_CARRO
INNER JOIN TELEFONE T
ON ID_CLIENTE = FK_ID_CLIENTE;
...
BEGIN
	...
	(NULL,OLD.ID_USUARIO,OLD.NOME,OLD.LOGIN);
END
$
...
BEGIN
	...
	INSERT INTO BACKUP.BKP_PRODUTO VALUES(NULL,NEW.ID_PRODUTO,
	NEW.NOME,NEW.VALOR);
...
BEGIN
	...
	INSERT INTO BACKUP.BKP_PRODUTO VALUES(NULL,OLD.ID_PRODUTO,
	OLD.NOME,OLD.VALOR);
...
/* QUANDO USAMOS O AFTER O ID_PRODUTO É COPIADO TAMBÉM, NO BEFORE O ID_PRODUTO FICA ZERO */
DELIMITER $
...
/*QUANDO => EXIBE A DATA E HORA ATUAL */
SELECT NOW();

/*QUEM => EXIBE O USUÁRIO CONECTADO */
SELECT CURRENT_USER();
...
CREATE TABLE BKP_PRODUTO(
	...
	/* DATETIME => GRAVA A DATA E A HORA */
	DATA DATETIME,
	USUARIO VARCHAR(30),
	EVENTO CHAR(3)
);
...
/* VALOR (TABELA PRODUTO) = VALOR_ORIGINAL (TABELA BKP_PRODUTO)*/
CREATE TRIGGER AUDIT_PROD
AFTER UPDATE ON PRODUTO
FOR EACH ROW
BEGIN

	INSERT INTO BACKUP.BKP_PRODUTO VALUES(NULL,OLD.ID_PRODUTO,OLD.NOME,
	OLD.VALOR,NEW.VALOR,NOW(),CURRENT_USER(),'UPD');
...
ALTER TABLE CURSOS ADD CONSTRAINT FK_PRE_REQ
FOREIGN KEY(FK_ID_PRE_REQ) REFERENCES CURSOS(ID_CURSO);
...
SELECT NOME, VALOR, HORAS, IFNULL(FK_ID_PRE_REQ,"SEM REQ") REQUISITO
FROM CURSOS;
...
C.VALOR AS VALOR, 
C.HORAS AS CARGA, 
IFNULL(P.NOME, "**SEM PRE-REQUISITO***") AS PREREQ
FROM CURSOS C LEFT JOIN CURSOS P
ON P.ID_CURSO = C.FK_ID_PRE_REQ;
...
SELECT NOME, (JAN+FEV+MAR) AS TOTAL, ROUND((JAN+FEV+MAR)/3, 2) AS MEDIA FROM VENDEDORES;
...
BEGIN
		...
		/* O CURSOR ARMAZENA DADOS NA VARIAVEL REG DO TIPO CURSOR */
		DECLARE REG CURSOR FOR(
			SELECT NOME, JAN, FEV, MAR FROM VENDEDORES
		);
		/* LOOP => ESTA VARIAVEL HANDLER FICA MONITORANDO SE A MANIPULACAO TERMINOU */
		DECLARE CONTINUE HANDLER FOR NOT FOUND SET FIM = 1;
		
		OPEN REG;
		
			REPEAT /* LOOP */
				/* FETCH = ME TRAGA O PROXIMO */
				/* ATENCAO! SEGUIR A ORDEM DA QUERY (NOME, JAN, FEV, MAR) */
				FETCH REG INTO VNOME, VJAN, VFEV, VMAR;
					IF NOT FIM THEN
					
						SET VTOTAL = VJAN + VFEV + VMAR;
						SET VMEDIA = ROUND((VTOTAL / 3),2);
						
						INSERT INTO VEND_TOTAL VALUES(VNOME,VJAN,VFEV,VMAR,VTOTAL,VMEDIA);
...
/* FICA VAZIO POIS TEMOS QUE RODAR O CURSOR ANTES */
...
BEGIN
		...
		INSERT INTO VEND_TOTAL VALUES(NEW.NOME,NEW.JAN,NEW.FEV,NEW.MAR,TOTAL,MEDIA);
...
CREATE TABLE INTERNACAO(
	...
	ENTRADA DATETIME,
	QUARTO INT,
	SAIDA DATETIME, /* DATA E HORA */
	OBSERVACOES VARCHAR(50),
	FK_ID_CONSULTA INT UNIQUE	
);  
...
SELECT CONSTRAINT_NAME, TABLE_NAME, CONSTRAINT_TYPE FROM TABLE_CONSTRAINTS;
